function DomainCtrl($scope,$rootScope, $http, $stateParams, Domain, Strip, Story) {
  ...
  //GET INFO domain
  Domain.returnInfo($http,domain).then(function(info) {
  ...
  // GET STORIES
  Story.returnStories($http,domain).then(function(stories) {
  ...
  $scope.showStory = function(storyId) {
    ...
    if($rootScope.idStories == 0) {
      Strip.returnAllStrips($http,domain).then(function(strips) {
        $scope.loading = false;
        $scope.strips = strips.data;
      });
    }
    else {
      Strip.returnStripsByStories($http,domain,$rootScope.idStories).then(function(strips) {
        $scope.loading = false;
        $scope.strips = strips.data;
      });
    }
